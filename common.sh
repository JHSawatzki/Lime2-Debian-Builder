...
install_board_specific (){
	...
	chroot $DEST/output/sdcard /bin/bash -c "apt-get -y -qq remove alsa-base && apt-get -y -qq autoremove"
	...
	# add sunxi tools
	cp $DEST/sunxi-tools/fex2bin $DEST/sunxi-tools/bin2fex $DEST/output/sdcard/usr/bin/
...
#EOT
	...
	cp $SRC/Lime2-Debian-Builder/scripts/sata-install.sh $DEST/output/sdcard/root
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /root/sata-install.sh"

	# copy and create symlink to default interfaces configuration
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/interfaces $DEST/output/sdcard/etc/network/
	...
	if [[ $BRANCH == *next* ]];then
		...
		cp $SRC/Lime2-Debian-Builder/config/boot-next.cmd $DEST/output/sdcard/boot/boot-next.cmd
		...
		fex2bin $SRC/Lime2-Debian-Builder/config/$BOARD.fex $DEST/output/sdcard/boot/$BOARD.bin
		cp $SRC/Lime2-Debian-Builder/config/boot.cmd $DEST/output/sdcard/boot/boot.cmd
	...
	unzip $SRC/Lime2-Debian-Builder/bin/linux-firmware.zip -d $DEST/output/sdcard/lib/firmware
}
...
create_debian_template (){
	...
	if [ ! -f "$DEST/output/rootfs/$RELEASE.raw.gz" ]; then
		...
		echo "Welcome to TMESLogger!" > $DEST/output/sdcard/etc/motd

		# choose proper apt list
		cp $SRC/Lime2-Debian-Builder/config/sources.list $DEST/output/sdcard/etc/apt/sources.list
...
END
		...
		chroot $DEST/output/sdcard /bin/bash -c "debconf-apt-progress -- apt-get -y install $PAKETE"
		
		#TODO WIFI
		#PAKETEWIFI="hostapd iw rfkill wireless-tools wpasupplicant"
		#chroot $DEST/output/sdcard /bin/bash -c "debconf-apt-progress -- apt-get -y install $PAKETEWIFI"
		...
		#echo T0:2345:respawn:/sbin/getty -L -a root ttyS0 115200 vt100 >> $DEST/output/sdcard/etc/inittab
		...
		cp $SRC/Lime2-Debian-Builder/scripts/resize2fs $DEST/output/sdcard/etc/init.d
		cp $SRC/Lime2-Debian-Builder/scripts/firstrun $DEST/output/sdcard/etc/init.d
		...
		cat $SRC/Lime2-Debian-Builder/scripts/bashrc >> $DEST/output/sdcard/etc/bash.bashrc

		# install ramlog only on wheezy
		cp $SRC/Lime2-Debian-Builder/bin/ramlog_2.0.0_all.deb $DEST/output/sdcard/tmp
...
do_tmeslogger_cutom (){
	# install watchdog
	chroot $DEST/output/sdcard /bin/bash -c "debconf-apt-progress -- apt-get -y install watchdog"
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/watchdog.conf $DEST/output/sdcard/etc/watchdog.conf

	# install acpi
	chroot $DEST/output/sdcard /bin/bash -c "debconf-apt-progress -- apt-get -y install acpid acpi-support-base"
	chroot $DEST/output/sdcard /bin/bash -c "service acpid stop"
	...
	chroot $DEST/output/sdcard /bin/bash -c "debconf-apt-progress -- apt-get -y install $PAKETE_TMESLOGGER"
	
	chroot $DEST/output/sdcard /bin/bash -c "service ssh stop"
	chroot $DEST/output/sdcard /bin/bash -c "service proftpd stop"
	chroot $DEST/output/sdcard /bin/bash -c "service nginx stop"
	chroot $DEST/output/sdcard /bin/bash -c "service php5-fpm stop"
	chroot $DEST/output/sdcard /bin/bash -c "service ntp stop"

	sed -e 's/^#Subsystem/Subsystem/g' -i $DEST/output/sdcard/etc/ssh/sshd_config
	
	cp $SRC/Lime2-Debian-Builder/config/if-up.d/ntp $DEST/output/sdcard/etc/network/if-up.d
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /etc/network/if-up.d/ntp"

	#Config user tmeslogger
	chroot $DEST/output/sdcard /bin/bash -c "adduser --system --home /usr/local/tmeslogger --group --disabled-password --disabled-login tmeslogger"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0770 /usr/local/tmeslogger"
	chroot $DEST/output/sdcard /bin/bash -c "usermod -a -G ftp tmeslogger"
	chroot $DEST/output/sdcard /bin/bash -c "usermod -a -G adm tmeslogger"
	chroot $DEST/output/sdcard /bin/bash -c "usermod -a -G tmeslogger ftp"
	chroot $DEST/output/sdcard /bin/bash -c "usermod -a -G tmeslogger www-data"
	chroot $DEST/output/sdcard /bin/bash -c "usermod -a -G sudo www-data"
	mkdir $DEST/output/sdcard/var/log/tmeslogger
	chroot $DEST/output/sdcard /bin/bash -c "chown tmeslogger:tmeslogger /var/log/tmeslogger/"
	
	cp $SRC/Lime2-Debian-Builder/scripts/dofstrim $DEST/output/sdcard/etc/cron.weekly/dofstrim
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /etc/cron.weekly/dofstrim"
	
	#Nginx Config
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/nginx.conf $DEST/output/sdcard/etc/nginx/nginx.conf
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/site-default $DEST/output/sdcard/etc/nginx/sites-available/default
	rm $DEST/output/sdcard/usr/share/nginx/www/50x.html
	rm $DEST/output/sdcard/usr/share/nginx/www/index.html
	cp $SRC/Lime2-Debian-Builder/config/if-up.d/nginx $DEST/output/sdcard/etc/network/if-up.d
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /etc/network/if-up.d/nginx"

	#PHP FPM Config
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/php-fpm.conf $DEST/output/sdcard/etc/php5/fpm/php-fpm.conf
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/php.ini $DEST/output/sdcard/etc/php5/fpm/php.ini
	

	#FTP Server
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/proftpd.conf $DEST/output/sdcard/etc/proftpd/proftpd.conf
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/modules.conf $DEST/output/sdcard/etc/proftpd/modules.conf
	rm $DEST/output/sdcard/srv/ftp/welcome.msg
	cp $SRC/Lime2-Debian-Builder/config/if-up.d/proftpd $DEST/output/sdcard/etc/network/if-up.d
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /etc/network/if-up.d/proftpd"
	chroot $DEST/output/sdcard /bin/bash -c "echo ftp:$ROOTPWD | chpasswd"
	chroot $DEST/output/sdcard /bin/bash -c "groupadd ftp"
	chroot $DEST/output/sdcard /bin/bash -c "usermod -g ftp ftp"
	chroot $DEST/output/sdcard /bin/bash -c "chown ftp:ftp /srv/ftp"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0770 /srv/ftp"

	#Logrotate
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/logrotate.conf $DEST/output/sdcard/etc/logrotate.conf
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/logrotate.d/rsyslog $DEST/output/sdcard/etc/logrotate.d/rsyslog
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/logrotate.d/nginx $DEST/output/sdcard/etc/logrotate.d/nginx
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/logrotate.d/proftpd-basic $DEST/output/sdcard/etc/logrotate.d/proftpd-basic
	/bin/cp -f $SRC/Lime2-Debian-Builder/config/logrotate.d/php5-fpm $DEST/output/sdcard/etc/logrotate.d/php5-fpm
	
	mkdir $DEST/output/sdcard/usr/local/tmeslogger/scripts
	chroot $DEST/output/sdcard /bin/bash -c "chown tmeslogger:tmeslogger /usr/local/tmeslogger/scripts"
	
	#Proprietary code
	if [[ $U6PRO == "yes" ]]; then
		cp $SRC/tmeslogger/tmeslogger_u6pro.sql $DEST/output/sdcard/usr/local/tmeslogger/tmeslogger.sql
	else
		cp $SRC/tmeslogger/tmeslogger.sql $DEST/output/sdcard/usr/local/tmeslogger/tmeslogger.sql
	fi

	#Setup db
	chroot $DEST/output/sdcard /bin/bash -c "sqlite3 /usr/local/tmeslogger/tmeslogger.db < /usr/local/tmeslogger/tmeslogger.sql"
	chroot $DEST/output/sdcard /bin/bash -c "chown tmeslogger:tmeslogger /usr/local/tmeslogger/tmeslogger.db"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0660 /usr/local/tmeslogger/tmeslogger.db"
	chroot $DEST/output/sdcard /bin/bash -c "rm /usr/local/tmeslogger/tmeslogger.sql"
	chroot $DEST/output/sdcard /bin/bash -c "sqlite3 /usr/local/tmeslogger/tmeslogger.db \"UPDATE loggerStatus SET tlvalue='$(date +%Y-%m-%d)' WHERE tlkey='currentDay'\""

	#Setup scripts
	cp $SRC/tmeslogger/scripts/batteryChecker.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/changeInterface.awk $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/changeNTP.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/changeSFTP.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/disableService.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/enableService.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/factoryReset.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/restartService.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/servicestatus.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/setDate.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/setNetwork.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/setPassword.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/setTimezone.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/startBlink.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/stopBlink.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/systemstatus.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts
	cp $SRC/tmeslogger/scripts/toggleNetwork.sh $DEST/output/sdcard/usr/local/tmeslogger/scripts

	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/batteryChecker.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0640 /usr/local/tmeslogger/scripts/changeInterface.awk"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/changeNTP.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/changeSFTP.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/disableService.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/enableService.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/factoryReset.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/restartService.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/servicestatus.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/setDate.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/setNetwork.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/setPassword.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/setTimezone.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/startBlink.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/stopBlink.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/systemstatus.sh"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0750 /usr/local/tmeslogger/scripts/toggleNetwork.sh"

	#Setup sudo
	cp $SRC/tmeslogger/scripts/tmeslogger.sudo $DEST/output/sdcard/etc/sudoers.d/tmeslogger
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0440 /etc/sudoers.d/tmeslogger"

	#Setup apps
	cp $SRC/tmeslogger/ftppush.py $DEST/output/sdcard/usr/local/tmeslogger/ftppush.py
	cp $SRC/tmeslogger/tmeslogger.py $DEST/output/sdcard/usr/local/tmeslogger/tmeslogger.py
	cp $SRC/tmeslogger/dbread.py $DEST/output/sdcard/usr/local/tmeslogger/dbread.py

	chroot $DEST/output/sdcard /bin/bash -c "chown tmeslogger:tmeslogger /usr/local/tmeslogger/ftppush.py"
	chroot $DEST/output/sdcard /bin/bash -c "chown tmeslogger:tmeslogger /usr/local/tmeslogger/tmeslogger.py"
	chroot $DEST/output/sdcard /bin/bash -c "chown tmeslogger:tmeslogger /usr/local/tmeslogger/dbread.py"

	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /usr/local/tmeslogger/ftppush.py"
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /usr/local/tmeslogger/tmeslogger.py"
	chroot $DEST/output/sdcard /bin/bash -c "chmod 0550 /usr/local/tmeslogger/dbread.py"

	cp $SRC/tmeslogger/scripts/ftppush $DEST/output/sdcard/etc/init.d/ftppush
	cp $SRC/tmeslogger/scripts/tmeslogger $DEST/output/sdcard/etc/init.d/tmeslogger
	cp $SRC/tmeslogger/scripts/processRebootAndShutdown $DEST/output/sdcard/etc/init.d/processRebootAndShutdown

	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /etc/init.d/ftppush"
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /etc/init.d/tmeslogger"
	chroot $DEST/output/sdcard /bin/bash -c "chmod +x /etc/init.d/processRebootAndShutdown"

	chroot $DEST/output/sdcard /bin/bash -c "update-rc.d ftppush defaults"
	chroot $DEST/output/sdcard /bin/bash -c "update-rc.d tmeslogger defaults"
	chroot $DEST/output/sdcard /bin/bash -c "update-rc.d processRebootAndShutdown defaults"

	#Web
	cp -r $SRC/tmeslogger/web/  $DEST/output/sdcard/usr/share/nginx/www/
	#chroot $DEST/output/sdcard /bin/bash -c "chown -cR www-data:www-data /usr/share/nginx/www"

	unzip $SRC/Lime2-Debian-Builder/bin/exodriver.zip -d $DEST/output/sdcard/root/
	unzip $SRC/Lime2-Debian-Builder/bin/LabJackPython.zip -d $DEST/output/sdcard/root/
	unzip $SRC/Lime2-Debian-Builder/bin/pyA20-0.2.0.zip -d $DEST/output/sdcard/root/

	#Install chilkat python library
	tar xzf $SRC/Lime2-Debian-Builder/bin/chilkat-9.5.0-python-2.7-armv7a-hardfp-linux.tar.gz
	mv $SRC/Lime2-Debian-Builder/bin/chilkat-9.5.0-python-2.7-armv7a-hardfp-linux/_chilkat.so $DEST/output/sdcard/usr/local/lib/python2.7/dist-packages/
	mv $SRC/Lime2-Debian-Builder/bin/chilkat-9.5.0-python-2.7-armv7a-hardfp-linux/chilkat.py $DEST/output/sdcard/usr/local/lib/python2.7/dist-packages/
	rm -r $SRC/Lime2-Debian-Builder/bin/chilkat-9.5.0-python-2.7-armv7a-hardfp-linux

	#Cron battery checker
	echo '* *     * * *   root    /usr/local/tmeslogger/scripts/batteryChecker.sh' >> $DEST/output/sdcard/etc/crontab
}
...
closing_image (){
	...
	rm $DEST/output/sdcard /usr/share/info/dir.old
	rm $DEST/output/sdcard/var/cache/debconf/*.dat-old
	rm $DEST/output/sdcard/var/log/{bootstrap,dpkg}.log
	rm $DEST/output/sdcard/var/log/*.?
	rm $DEST/output/sdcard/tmp/*
	for a in $DEST/output/sdcard/var/log/{*.log,apt/*.log,debug,dmesg,faillog,messages,syslog,wtmp} do > $a; done
	rm $DEST/output/sdcard/var/cache/apt/* 
	rm $DEST/output/sdcard/var/lib/apt/lists/*

	rm $DEST/output/sdcard/etc/adjtime
